...
- ✅ **本地 HTML 报告**：默认随终端报告保存但不打开，使用 `--web` 自动打开，CLI 退出后仍可查看
- ✅ **官网归仓**：历史官网源码和 GitHub Pages 部署迁入主仓库，保留旧结果链接
- ✅ **双语与响应式**：页面内切换中英文，适配桌面和移动端
- ✅ **时间图表**：24 小时提交脉冲、星期分布、工作日/周末/深夜证据
- ✅ **多仓库对比**：保留成功、失败、开源不评分和低样本状态
- ⏳ **进一步导出**：PDF、PNG、CSV 尚未实现
