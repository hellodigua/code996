...
核心算法使用 committer date，默认忽略 merge commit。时间桶使用 `%cd`，时区过滤和主时区摘要使用同一提交的 `%ci`，不能混用 author date。项目分类、时区过滤和中国节假日模式会影响结果是否展示以及工作日/休息日划分。
...
- 每日末次提交把 `00:00–05:59` 视为前一工作日的延续，并以 `1440–1799` 分钟保存；需要展示或导出时使用 `24:00–29:59` 扩展时间，不能截断为 `23:59`。
