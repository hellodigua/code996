...
│       ├── multi.ts       # 多仓库分析命令 - 扫描、选择、合并多个仓库数据
│       ├── report/        # 报表输出模块（新增）
│       │   ├── index.ts   # 统一导出接口
...
- **参数解析**：支持年份快捷方式（`-y`）、日期范围（`--since/--until`）、全量分析（`--all-time`）、半小时粒度展示（`--half-hour`）、多仓库扫描（`--max`）等选项
- **用户交互**：提供彩色输出、自适应表格和进度指示器
- **多仓库支持**：`multi` 命令支持交互式仓库选择、批量数据采集和结果合并
- **报表输出**：模块化的报表系统，支持核心结果、详细分析、时间分布（24小时/48半小时双模式）、加班分析等多维度展示

### 2. 数据采集层 (Git Collector)

- **数据收集**: 收集仓库的所有提交数据
- **Git命令执行**: 执行 git log、git rev-list 等命令获取提交数据，采用分钟级时间格式（`%H:%M`）
