...
在实际工作中，一个人可能同时参与多个项目。`--repos` 参数可以将多个仓库的提交数据合并分析，全面评估整体工作强度。

### 功能特点

- **数据合并**：自动合并多个仓库的提交记录，按小时/星期统计
- **智能去重**：按日期合并每日最早/最晚提交时间
- **保持一致**：使用相同的时间范围和作者过滤规则
- **进度展示**：逐个仓库采集并显示进度
...
```bash
# 基础多仓库分析
code996 --repos "/path/repo1,/path/repo2,/path/repo3"

# 带时间范围
code996 --repos "D:\project1,D:\project2" -y 2024

# 组合自定义下班时间
code996 --repos "/path/repo1,/path/repo2" --end-hour 18 --all-time

# 组合作者过滤
code996 --repos "/path/repo1,/path/repo2" --self --exclude-authors bot
```
