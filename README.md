...
- **🖥️ 本地 Web 报告**：使用 `--web` 生成并打开中英双语可视化报告，无需上传数据或常驻本地服务
...
默认会直接在终端输出传统文字报告。需要可视化结果时运行 `npx code996 --web`；报告所需的 HTML、脚本、样式和字体会一起生成到 `Downloads/code996-report/日期_时间_项目名/`，例如 `Downloads/code996-report/2026-07-18_10-11-46_demo/`。CLI 退出后仍可继续查看，删除对应目录即可完整移除报告；整个过程不会上传仓库数据，也不需要启动 localhost 服务。
...
| 默认              | 在终端输出传统文字报告                                         |
| `--web`           | 生成并打开本地 Web 报告                                        |
| `--web --no-open` | 生成 Web 报告但不自动打开浏览器，并在终端打印 HTML 路径        |
| `--json` / `--md` | 输出稳定的 JSON 数据或 Markdown 表格，不打开网页               |
| `--output [path]` | 将 `--json` 或 `--md` 保存到指定路径；省略路径时使用默认文件名 |

`--json`、`--md` 和 `--web` 互斥，`--no-open` 只能与 `--web` 一起使用。Web 报告支持在页面右上角直接切换中文和英文；`--lang` 决定初始语言。
...
# 报告输出
code996                        # 默认使用传统终端报表
code996 --web                  # 生成并打开 Web 报告
code996 --web --no-open        # 只生成报告，打印本地 HTML 路径
...
3. 默认输出终端报告，或根据显式参数生成 Web / JSON / Markdown 结果
