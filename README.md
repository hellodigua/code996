...
### Q: 项目习惯本地多次提交后 Squash 再推送，会影响准确性吗？

影响会有，但 code996 依赖的是统计学规律，只要 commit 样本达到一定数量，最终呈现的是团队整体的集体工作模式，特殊提交习惯在统计学上会被视为“噪声”被过滤掉。

### Q: 跨时区项目是否支持使用 code996？

完美支持！Git 提交记录自带本地时区信息，同时工具默认也会自动分析项目的时区分布并进行提示，防止数据混淆。

同时支持使用 --timezone="+0800" 等参数手动筛选特定地区，实现对跨国团队的精准侦察。

### Q: 为什么不支持查看项目成员的996指数？
